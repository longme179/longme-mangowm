...
#
# Cập nhật:
# 1. Sửa triệt để bug Race Condition của GTK D&D: Trì hoãn việc reset cờ `is_dragging`
#    sau khi drop để chặn sự kiện `button-release-event` bị rò rỉ. Điều này giúp:
#    - Không tự động chuyển workspace sau khi kéo thả.
#    - Không tự động đóng GUI sau khi kéo thả.
# 2. Đảm bảo UI luôn mở để người dùng thao tác liên tục nhiều app, chỉ đóng khi:
#    - Nhấn ESC.
#    - Click vào vùng trống (background).
#    - Click vào Workspace (chuyển view).
#    - Click vào App (chuyển view & focus).
# =============================================================================
...
def get_icon_name(app_id: str, title: str) -> str:
    """Lấy icon an toàn dựa vào Theme hiện hành."""
    if not app_id:
    ...
    if theme.has_icon(app_id): return app_id
    if theme.has_icon(app_id_lower): return app_id_lower
...
class AppWidget(Gtk.EventBox):
    ...
    def on_button_release(self, widget, event):
        # Tránh trigger nếu vừa mới kết thúc thao tác drag-and-drop
        if event.button == 1 and not self.main_window.is_dragging:
            # Single-click: chuyển workspace, focus app, đóng grid.
            MangoWM_IPC.dispatch([f"view,{self.client.ws},0"])
    ...
    def on_drag_end(self, widget, context):
        ...
        # FIX RACE CONDITION: Delay việc tắt cờ is_dragging để ngăn chặn
        # GTK `button-release-event` gọi nhầm các hàm single-click/workspace-click.
        GLib.timeout_add(150, self.main_window.clear_dragging_state)
...
class WorkspaceWidget(Gtk.EventBox):
    ...
    def __init__(self, ws_id: int, main_window: "GridOverview"):
        ...
        self.box.set_size_request(240, 200)
    ...
    def on_button_release(self, widget, event):
        # Sẽ bị bypass nếu event release này là do hệ quả của quá trình Drag&Drop
        if event.button == 1 and not self.main_window.is_dragging:
    ...
    def on_drag_data_received(self, widget, context, x, y, data, info, time):
        ...
        if app_id:
            # Gán tag mới cho Client, KHÔNG chuyển view (không view,{ws_id})
            MangoWM_IPC.dispatch([f"tag,{self.ws_id},0", f"client,{app_id}"])
            context.finish(True, False, time)

            GLib.timeout_add(150, self.main_window.sync_state)
        else:
...
class GridOverview(Gtk.Window):
    def __init__(self):
        ...
        self.hint_label = Gtk.Label(label="Kéo thả App để di chuyển • Click App để focus • Click Workspace để chuyển • ESC để thoát")
    ...
    def clear_dragging_state(self) -> bool:
        """Được gọi qua timeout sau khi Drag&Drop kết thúc hoàn toàn."""
        self.is_dragging = False
        return False
